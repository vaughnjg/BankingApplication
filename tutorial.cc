...
public:
    ...
    virtual void OnUnitIdle(const Unit* unit) final {
        ...
        case UNIT_TYPEID::TERRAN_SCV: {
            ...
            const Unit* vespene_target = FindNearestVespeneGas(unit->pos);
            if (!mineral_target) {
                break;
            }
            if (!vespene_target) {
...
private:
    ...
    bool TryBuildSupplyDepot() {
        ...
        // If we are less than 80 % capacity, do not build a supply depot
        if (observation->GetFoodUsed() <= observation->GetFoodCap() * 0.8)
            return false;
    ...
    bool TryBuildVespeneGas() {
        ...
        if (CountUnitType(UNIT_TYPEID::NEUTRAL_VESPENEGEYSER) > 2) {
            return false;
        }
